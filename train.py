...
from data.dataset import get_bow_data_loaders, get_tfidf_data_loaders, get_corpus, CommentDataset, get_rnn_data_loaders, \
    get_rnn_corpus, get_lstm_data_loaders
...
def train_rnn():
    ...
    for epoch in range(EPOCHES):
        ...
        threshold, f1_val, f1s = find_best_threshold(rnn, val_loader, device)
...
def train_lstm():
    ...
    print("creating model")
    lstm = LSTMModel(len(vocabulary))
    lstm = lstm.to(device)

    print("Running LSTM on " + device)

    LSTMTrainer = Trainer(lstm, torch.optim.Adam(lstm.parameters()),
                          # nn.BCEWithLogitsLoss(pos_weight=torch.tensor([8.9], device=device)),
                          nn.BCEWithLogitsLoss(),
                          device=device)
    ...
    for epoch in range(EPOCHES):
        lstm.train()
        print(f"Loss: {LSTMTrainer.train_epoch(train_loader)}")
        # torch.save(RNN_trainer.model.state_dict(), Path("saves") / "goida")
        # probs = torch.linspace(0.005, 0.99, 200)
        # accuracy, recall, precision, f1, prob = get_metrics(rnn, val_loader, test_loader, device=device)
        # accuracy, recall, precision, f1, prob = get_metrics(rnn, train_loader, train_loader, device=device)
        ...
        threshold, f1_val, f1s = find_best_threshold(lstm, val_loader, device)
        ...
        with torch.no_grad():
            ...
            for X_batch, y_batch in test_loader:
                ...
                preds = torch.sigmoid(lstm(X_batch).reshape(-1))
                # print(preds)
